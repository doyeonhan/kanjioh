(function (window, $, undefined) {
  ...
  questionCdn  = -1;
	...
	function htmlInit(proc){
		...
		kanjiJson = getJsonData();

		for(var key in kanjiJson){
		   questionList.push(kanjiJson[key][1]);
	  	}
	...
	window.koh = {
	    ...
	    changeForm: function(type){
	    	...
	    	var getSpan = function(type, d, dic, idx) {
				var aaa = [dic[d][0], dic[d][1], "&nbsp;", dic[d][0] + " " + dic[d][1]];
				var kanji = "&nbsp;";
				var result = "";
				...
				result += "&nbsp;<span style='position: relative; display: inline-block; width: 80px; line-height: 15px; top: 27px; white-space:nowrap; text-align: center;'>" + kanji + "<hr/><a style='position: relative; font-size: 12px; top: -10px;'>" + aaa[type] + "</a></span>&nbsp;";
			...
			var idx = 0;
			for(d in dic) {			
				ar = ar.replace(new RegExp(d, 'gi'), getSpan(type, d, dic));
				idx++;
			}	
